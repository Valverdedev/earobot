---
name: analisar-eurusd
description: Analise profissional e auditavel de EURUSD para este robo, combinando Price Action, indicadores, dados do broker, noticias e fundamentos com fontes. Use ao gerar ou atualizar relatorios de EUR/USD, decidir entre manter ou refazer a tese fundamental, selecionar uma estrategia registrada em docs/catalogo-estrategias.md, preparar um config candidato e, sob autorizacao e controles de seguranca, promover o config para D:\SistemEarobot\config.
---
...
Produza uma decisao operacional rastreavel para o par EUR/USD do broker. Trate a saida como apoio a decisao, nunca como garantia de retorno. Admita `NAO OPERAR` quando dados, contexto, liquidez, risco de evento ou compatibilidade do codigo forem insuficientes.
...
- `references/eurusd-market-framework.md`
- `references/report-template.md`
- `D:\SistemEarobot\financial.robot\docs\catalogo-estrategias.md`

## Caminhos fixos

- Relatorios: `D:\SistemEarobot\relatorios\EURUSD`
- Prompt operacional: `D:\SistemEarobot\relatorios\EURUSD\prompt_analise_eurusd.md`
...
- Config ativo: `D:\SistemEarobot\config\EURUSD.config.json`
- Terminal padrao: `activtraders`
- Simbolo padrao do broker: `EURUSD`
...
6. Nao presuma que especificacoes do futuro Euro FX da CME sejam iguais as do CFD/spot `EURUSD` do broker. Use metadados extraidos do simbolo para point size, digits, spread, volume e horario.
...
```powershell
& "D:\SistemEarobot\financial.robot\.agents\skills\analisar-eurusd\scripts\Prepare-EurusdAnalysis.ps1" -RunExtractor
```

O script localiza o relatorio anterior por nome exato, prepara os nomes da nova execucao e extrai `M1,M5,M15,H1,H4,D1` com 500 candles. Capture o objeto retornado. O relatorio deve se chamar `yyyy-MM-dd_HH-mm-ss_eurusd.md`; nao use dois-pontos no nome.
...
- houve CPI, PPI, payroll, FOMC, ECB, decisao de juros, coletiva relevante, PMI, PIB, inflacao HICP, emprego, vendas, ISM ou dado fiscal/material;
...
- iniciou nova sessao relevante (Asia, Londres, New York, overlap Londres/NY), nova semana, ou ocorreu fechamento/reabertura de fim de semana;
...
- se spread e stop minimo sao compativeis com o ATR do timeframe escolhido.
...
Pesquise fontes atuais na internet. Priorize fontes primarias: Federal Reserve, ECB, Eurostat, BLS, BEA, U.S. Treasury, FRED, CFTC, CME, bancos centrais nacionais relevantes e orgaos oficiais. Use Reuters ou AP para noticia em tempo real e geopolitica, preferencialmente com corroboracao.
...
- diferencial de politica monetaria Fed x ECB;
- juros nominais e reais dos EUA e, quando disponivel, yields da Alemanha/euro area;
- dolar, condicoes financeiras e apetite por risco;
- inflacao, emprego, atividade, PMI/ISM e crescimento nos EUA e na zona do euro;
- termos de troca, energia, risco geopolitico europeu e fiscal;
- fluxos/posicionamento quando houver fonte atual, como CFTC COT para Euro FX;
- calendario de alto impacto e janela ate o proximo evento.

### 6. Calcular e ler o grafico

Calcule as metricas de `references/eurusd-market-framework.md` a partir do JSON. Faca leitura top-down nesta ordem: `D1 -> H4 -> H1 -> M15 -> M5 -> M1`.
...
Produza no minimo cenarios altista para EUR, baixista para EUR e neutro. Para cada um, informe:
...
- trate `AberturaFaixaRompimento`/`OpeningRangeBreakout` como agressiva e sensivel a reinicio/historico;
...
Promova para `D:\SistemEarobot\config\EURUSD.config.json` somente quando todas as condicoes forem verdadeiras:
