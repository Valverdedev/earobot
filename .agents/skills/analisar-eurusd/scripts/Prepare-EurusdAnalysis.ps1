[CmdletBinding()]
param(
    [string]$ReportDirectory = "D:\SistemEarobot\relatorios\EURUSD",
...
$reportPattern = '^\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}_eurusd\.md$'
...
do {
    ...
    $reportPath = Join-Path $ReportDirectory "${timestamp}_eurusd.md"
    $extractionPath = Join-Path $ReportDirectory "${timestamp}_eurusd_mt5.json"
    $candidateConfigPath = Join-Path $ReportDirectory "${timestamp}_eurusd_config.candidate.json"
...
[PSCustomObject]@{
    ...
    ActiveConfigPath = 'D:\SistemEarobot\config\EURUSD.config.json'
