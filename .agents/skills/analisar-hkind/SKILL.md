---
name: analisar-hkind
description: Analise profissional e auditavel de HKInd para este robo, combinando Price Action, indicadores, dados do broker, noticias e fundamentos com fontes. Use ao gerar ou atualizar relatorios de HKInd/Hang Seng, decidir entre manter ou refazer a tese fundamental, selecionar uma estrategia registrada em docs/catalogo-estrategias.md, preparar um config candidato e, sob autorizacao e controles de seguranca, promover o config para D:\SistemEarobot\config.
---
...
Produza uma decisao operacional rastreavel para o simbolo HKInd do broker. Trate a saida como apoio a decisao, nunca como garantia de retorno. Admita `NAO OPERAR` quando dados, contexto, liquidez, risco de evento, feriado/sessao asiatica, risco geopolitico, qualidade de dados ou compatibilidade do codigo forem insuficientes.
...
- `references/hkind-market-framework.md`
- `references/report-template.md`
- `D:\SistemEarobot\financial.robot\docs\catalogo-estrategias.md`

## Caminhos fixos

- Relatorios: `D:\SistemEarobot\relatorios\HKIND`
- Prompt operacional: `D:\SistemEarobot\relatorios\HKIND\prompt_analise_hkind.md`
...
- Config ativo: `D:\SistemEarobot\config\HKIND.config.json`
- Terminal padrao: `activtraders`
- Simbolo padrao do broker: `HKInd`
...
6. Nao presuma que especificacoes do Hang Seng Index, Hang Seng futures da HKEX ou outro CFD de Hong Kong sejam iguais ao `HKInd` do broker. Use metadados extraidos do simbolo para point size, digits, spread, volume, stop level e horario.
7. Valide sessao de Hong Kong, feriados, almoco/intervalos, liquidez de pre/pos-abertura, gaps e eventual ajuste/rolagem do simbolo do broker antes de operar.
...
```powershell
& "D:\SistemEarobot\financial.robot\.agents\skills\analisar-hkind\scripts\Prepare-HKIndAnalysis.ps1" -RunExtractor
```

O script localiza o relatorio anterior por nome exato, prepara os nomes da nova execucao e extrai `M1,M5,M15,H1,H4,D1` com 500 candles. Capture o objeto retornado. O relatorio deve se chamar `yyyy-MM-dd_HH-mm-ss_hkind.md`; nao use dois-pontos no nome.
...
- houve decisao/comunicado do PBoC, HKMA ou Fed, CPI/PPI/PMI/PIB/producao/vendas/credito da China, Hong Kong ou EUA, payroll, CPI/PCE/FOMC, ou dado relevante de comercio exterior;
- houve choque geopolitico, regulatorio, imobiliario, bancario, fiscal, cambial ou acao oficial relevante envolvendo China/Hong Kong/EUA;
- houve movimento material em Hang Seng, Hang Seng Tech, China A-shares, USDHKD, CNH/CNY, US yields, Nasdaq/S&P ou apetite por risco global;
...
- iniciou sessao relevante de Hong Kong/Asia, reabertura apos intervalo, Londres, New York, nova semana, feriado ou reabertura apos fim de semana;
- mudou a liquidez por feriado, meia sessao, rollover/ajuste do broker ou horario de verao/inverno;
...
- ordem temporal, duplicidades e gaps, especialmente na abertura de Hong Kong, no retorno do intervalo e apos fechamento/reabertura;
...
- se `HKInd` esta em periodo normal de negociacao, sem feriado, meia sessao, rollover/ajuste do broker ou simbolo obsoleto.

Se houver dados stale, spread invalido, gaps materiais, feriado/sessao problematica ou amostra insuficiente, reduza a confianca. Bloqueie novas entradas quando a falha puder mudar a direcao, os niveis ou o dimensionamento.
...
Pesquise fontes atuais na internet. Priorize fontes primarias: Hang Seng Indexes, HKEX, HKMA, Hong Kong Census and Statistics Department, Hong Kong Government, PBoC, National Bureau of Statistics of China, SAFE, China Customs, Federal Reserve, BLS/BEA e U.S. Treasury. Use Reuters ou AP para noticia em tempo real e geopolitica, preferencialmente com corroboracao.
...
- politica monetaria Fed, PBoC e HKMA, USDHKD peg, CNH/CNY e yields dos EUA;
- dados de Hong Kong e China: CPI/PPI, PMI, PIB, retail sales, industrial production, credit impulse, comercio exterior, emprego e setor imobiliario;
- apetite por risco global e tecnologia: Hang Seng Tech, A-shares, Nasdaq/S&P, VIX e noticias de ADRs/Big Tech chinesas;
- geopolitica, regulacao, tarifas, restricoes tecnologicas e fluxo China/Hong Kong;
- composicao setorial do Hang Seng como contexto, sem transformar isso em sinal intraday isolado;
- agenda de alto impacto e janela ate o proximo evento;
- feriados, sessoes parciais, abertura/intervalo/fechamento de Hong Kong e eventuais ajustes do simbolo `HKInd`.

### 6. Calcular e ler o grafico

Calcule as metricas de `references/hkind-market-framework.md` a partir do JSON. Faca leitura top-down nesta ordem: `D1 -> H4 -> H1 -> M15 -> M5 -> M1`.
...
Produza no minimo cenarios altista para HKInd, baixista para HKInd e neutro. Para cada um, informe condicoes necessarias, gatilho observavel, nivel de invalidacao, objetivos tecnicos, riscos de evento/spread/sessao, timeframe de validade e estrategia candidata ou `nenhuma`.

Nao converta o cenario base em ordem quando a relacao risco/retorno, o spread, a proximidade de noticia de alto impacto, abertura/reabertura de Hong Kong ou qualidade dos dados forem inadequados.
...
- trate `AberturaFaixaRompimento`/`OpeningRangeBreakout` como agressiva e sensivel a reinicio/historico; no HKInd, use apenas se a faixa de abertura/reabertura de Hong Kong estiver bem definida e reconstruivel;
- calibre canais e niveis fixos de `HydrusEmaChannelBreakout` com dados atuais do `HKInd`;
- use `ReversaoRange` apenas em range estabelecido e fora de janela de rompimento de abertura/noticia;
...
Promova para `D:\SistemEarobot\config\HKIND.config.json` somente quando todas as condicoes forem verdadeiras:
...
5. Nao ha evento de alto impacto iminente nem bloqueio de qualidade/sessao/ajuste. Spread alto isolado nao e bloqueio se houver filtro operacional de spread coerente e validado para impedir entradas no momento da ordem.
...
- reabertura do mercado apos fim de semana ou feriado;
- proxima janela de ajuste/rollover do broker se ela vier antes.

Inclua tambem gatilhos antecipados: movimento maior que 1 ATR H1, rompimento de nivel-chave, mudanca de regime, spread anormal sem filtro, feriado/sessao problematica, ajuste/rollover, dado stale ou noticia material. O horario deve ser absoluto, com fuso; nao escreva apenas `daqui a 4 horas`.
