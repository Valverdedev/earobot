[CmdletBinding()]
param(
    [string]$ReportDirectory = "D:\SistemEarobot\relatorios\HKIND",
...
$reportPattern = '^\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}_hkind\.md$'
...
do {
    ...
    $reportPath = Join-Path $ReportDirectory "${timestamp}_hkind.md"
    $extractionPath = Join-Path $ReportDirectory "${timestamp}_hkind_mt5.json"
    $candidateConfigPath = Join-Path $ReportDirectory "${timestamp}_hkind_config.candidate.json"
...
[PSCustomObject]@{
    ...
    ActiveConfigPath = 'D:\SistemEarobot\config\HKIND.config.json'
