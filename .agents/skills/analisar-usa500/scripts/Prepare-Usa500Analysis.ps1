[CmdletBinding()]
param(
    [string]$ReportDirectory = "D:\SistemEarobot\relatorios\USA500",
...
$reportPattern = '^\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}_usa500\.md$'
...
$latestReport = Get-ChildItem -LiteralPath $ReportDirectory -File |
    ...
    ForEach-Object {
        ...
        $isValidTimestamp = [DateTime]::TryParseExact($timestampText, $timestampPattern, $culture, [System.Globalization.DateTimeStyles]::None, [ref]$parsedTimestamp)
        if ($isValidTimestamp) {
...
do {
    ...
    $reportPath = Join-Path $ReportDirectory "${timestamp}_usa500.md"
    $extractionPath = Join-Path $ReportDirectory "${timestamp}_usa500_mt5.json"
    $candidateConfigPath = Join-Path $ReportDirectory "${timestamp}_usa500_config.candidate.json"
...
[PSCustomObject]@{
    ...
    ActiveConfigPath = 'D:\SistemEarobot\config\USA500.config.json'
