---
name: analisar-usa500
description: Analise profissional e auditavel de Usa500 para este robo, combinando Price Action, indicadores, dados do broker, noticias e fundamentos com fontes. Use ao gerar ou atualizar relatorios de Usa500/S&P 500, decidir entre manter ou refazer a tese fundamental, selecionar uma estrategia registrada em docs/catalogo-estrategias.md, preparar um config candidato e, sob autorizacao e controles de seguranca, promover o config para D:\SistemEarobot\config.
---
...
Produza uma decisao operacional rastreavel para o simbolo Usa500 do broker. Trate a saida como apoio a decisao, nunca como garantia de retorno. Admita `NAO OPERAR` quando dados, contexto, liquidez, risco de evento, sessao dos EUA, earnings, qualidade de dados ou compatibilidade do codigo forem insuficientes.
...
- `references/usa500-market-framework.md`
- `references/report-template.md`
- `D:\SistemEarobot\financial.robot\docs\catalogo-estrategias.md`

## Caminhos fixos

- Relatorios: `D:\SistemEarobot\relatorios\USA500`
- Prompt operacional: `D:\SistemEarobot\relatorios\USA500\prompt_analise_usa500.md`
...
- Config ativo: `D:\SistemEarobot\config\USA500.config.json`
- Terminal padrao: `activtraders`
- Simbolo padrao do broker: `Usa500`
...
6. Nao presuma que especificacoes do S&P 500 cash, E-mini S&P 500 futures/CME ou ETF SPY sejam iguais ao `Usa500` do broker. Use metadados extraidos do simbolo para point size, digits, spread, volume, stop level e horario.
7. Valide sessao regular dos EUA, pre-market/after-hours, feriados, meia sessao, earnings e eventos macro antes de operar.
...
```powershell
& "D:\SistemEarobot\financial.robot\.agents\skills\analisar-usa500\scripts\Prepare-Usa500Analysis.ps1" -RunExtractor
```

O script localiza o relatorio anterior por nome exato, prepara os nomes da nova execucao e extrai `M1,M5,M15,H1,H4,D1` com 500 candles. Capture o objeto retornado. O relatorio deve se chamar `yyyy-MM-dd_HH-mm-ss_usa500.md`; nao use dois-pontos no nome.
...
- houve FOMC, ata, discurso material do Fed, CPI, PPI, PCE, payroll, jobless claims, ISM/PMI, retail sales, GDP, Beige Book, leilao do Treasury, earnings relevantes ou guidance de mega caps;
- houve choque geopolitico, fiscal, bancario, liquidez/credito, energia, IA/semicondutores, tarifas ou acao oficial relevante;
- houve movimento material em US yields, DXY, VIX, Nasdaq, Russell, credit spreads, oil ou futuros globais;
...
- iniciou sessao relevante: pre-market EUA, cash open NYSE/Nasdaq, lunch, power hour, after-hours, nova semana ou reabertura apos fim de semana/feriado;
...
- ordem temporal, duplicidades e gaps, especialmente em pre-market, cash open, fechamento e reabertura;
...
- se `Usa500` esta em periodo normal de negociacao, sem feriado, meia sessao, rollover/ajuste do broker ou simbolo obsoleto.
...
Pesquise fontes atuais na internet. Priorize fontes primarias: Federal Reserve, FOMC, BLS, BEA, Census, ISM, S&P Global PMI, U.S. Treasury, CBO, CME, Cboe, NYSE/Nasdaq, S&P Dow Jones Indices e releases oficiais de empresas. Use Reuters ou AP para noticia em tempo real e geopolitica, preferencialmente com corroboracao.
...
- politica monetaria Fed, yields, Treasury auctions, DXY e condicoes financeiras;
- inflacao, emprego, consumo, atividade, credito e earnings;
- apetite por risco: Nasdaq, Russell, VIX, credit spreads, dolar, oil e futuros globais;
- concentracao setorial e mega caps do S&P 500 como contexto, sem transformar isso em sinal intraday isolado;
- calendario macro e earnings de alto impacto e janela ate o proximo evento;
- sessao, feriados, meia sessao, cash open/close e eventuais ajustes do simbolo `Usa500`.

### 6. Calcular e ler o grafico

Calcule as metricas de `references/usa500-market-framework.md` a partir do JSON. Faca leitura top-down nesta ordem: `D1 -> H4 -> H1 -> M15 -> M5 -> M1`.
...
Produza no minimo cenarios altista para Usa500, baixista para Usa500 e neutro. Para cada um, informe condicoes necessarias, gatilho observavel, nivel de invalidacao, objetivos tecnicos, riscos de evento/spread/sessao, timeframe de validade e estrategia candidata ou `nenhuma`.
...
- trate `AberturaFaixaRompimento`/`OpeningRangeBreakout` como agressiva e sensivel a reinicio/historico; no Usa500, use apenas se a faixa de abertura dos EUA estiver bem definida e reconstruivel;
- calibre canais e niveis fixos de `HydrusEmaChannelBreakout` com dados atuais do `Usa500`;
...
Promova para `D:\SistemEarobot\config\USA500.config.json` somente quando todas as condicoes forem verdadeiras:
