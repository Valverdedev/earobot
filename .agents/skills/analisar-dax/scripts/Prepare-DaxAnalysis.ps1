[CmdletBinding()]
param(
    [string]$ReportDirectory = "D:\SistemEarobot\relatorios\DAX",
...
$reportPattern = '^\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}_dax\.md$'
...
do {
    ...
    $reportPath = Join-Path $ReportDirectory "${timestamp}_dax.md"
    $extractionPath = Join-Path $ReportDirectory "${timestamp}_dax_mt5.json"
    $candidateConfigPath = Join-Path $ReportDirectory "${timestamp}_dax_config.candidate.json"
...
[PSCustomObject]@{
    ...
    ActiveConfigPath = 'D:\SistemEarobot\config\DAX.config.json'
