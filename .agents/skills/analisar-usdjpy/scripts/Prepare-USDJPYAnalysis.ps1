[CmdletBinding()]
param(
    [string]$ReportDirectory = "D:\SistemEarobot\relatorios\USDJPY",
...
$reportPattern = '^\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}_usdjpy\.md$'
...
$latestReport = Get-ChildItem -LiteralPath $ReportDirectory -File |
    ...
    ForEach-Object {
        ...
        if ([DateTime]::TryParseExact($timestampText, $timestampPattern, $culture, [System.Globalization.DateTimeStyles]::None, [ref]$parsedTimestamp)) {
...
do {
    ...
    $reportPath = Join-Path $ReportDirectory "${timestamp}_usdjpy.md"
    $extractionPath = Join-Path $ReportDirectory "${timestamp}_usdjpy_mt5.json"
    $candidateConfigPath = Join-Path $ReportDirectory "${timestamp}_usdjpy_config.candidate.json"
...
[PSCustomObject]@{
    ...
    ActiveConfigPath = 'D:\SistemEarobot\config\USDJPY.config.json'
